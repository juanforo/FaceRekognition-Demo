      ...
      snapshot.upload({api_url: api_url}).done(function(response) {
        ...
        if (data.id !== undefined && data.id != "0" && data.id != "UNRECOGNIZED") {
            ...
            //if Hollman
                ...
                intervalManager(false);
                // create speech response
                $.post("/speech", {tosay: "Good " + greetingTime(moment()) + " " + data.id + ". Welcome to eendava. Today you have no new tickets, 1, ongoing ticket,and you were assigned to the buzzpoints project, Also please remember to fill your oracle timesheets, and log your travel expenses"}, function(response) {
                    $("#audio_speech").attr("src", "data:audio/mpeg;base64," + response);
                    $("#audio_speech")[0].play();
                });
            }
